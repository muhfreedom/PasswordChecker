...
function Parse_Common5()
{
   ...
   {
      window.setTimeout('Parse_Common5()', 20);
   }
   else
   {
...
// number for "qu".
function Parse_Frequency_Token5()
{
   var c;
...
function Get_Charset_Size5(pass)
{
...
function Set_Text5(s)
{
   ...
   e = document.getElementById('passchk_result5');
   if (! e)
   {
      return;
   }
...
function ShowStats5()
{
   ...
   if (pass == OldPass5)
   {
      window.setTimeout('ShowStats5();', 200);
      return;
   }
   OldPass5 = pass;
   ...
   if (pass.length > 1)
   {
      ...
      charSet = Math.log(Get_Charset_Size5(pass)) / Math.log(2);
      aidx = Get_Index5(plower.charAt(0));
      for (var b = 1; b < plower.length; b ++)
      {
	 var bidx = Get_Index5(plower.charAt(b));
...
function CheckIfLoaded5()
{
   ...
   else if (! document.Common_Parsed)
   {
      ...
      {
         window.setTimeout('Parse_Common5()', 50);
   ...
   {
      Set_Text5(s + "Loading ...");
      window.setTimeout('CheckIfLoaded5()', 200);
      return;
   }
   
   // Loaded. Do initialization thingies.
   Set_Text5("Finished Loading.");
   window.setTimeout('ShowStats5();', 1000);
}

window.setTimeout('CheckIfLoaded5()', 100);
