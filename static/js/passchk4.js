...
function Parse_Common4()
{
   ...
   {
      window.setTimeout('Parse_Common4()', 20);
   }
   else
   {
...
// number for "qu".
function Parse_Frequency_Token4()
{
   var c;
...
function Get_Charset_Size4(pass)
{
...
function Set_Text4(s)
{
   ...
   e = document.getElementById('passchk_result4');
   if (! e)
   {
      return;
   }
...
function ShowStats4()
{
   ...
   if (pass == OldPass4)
   {
      window.setTimeout('ShowStats4();', 200);
      return;
   }
   OldPass4 = pass;
   ...
   if (pass.length > 1)
   {
      ...
      charSet = Math.log(Get_Charset_Size4(pass)) / Math.log(2);
      aidx = Get_Index4(plower.charAt(0));
      for (var b = 1; b < plower.length; b ++)
      {
	 var bidx = Get_Index4(plower.charAt(b));
	 ...
	 r += "";
      ...
      //r += "Charset Size:  " + Get_Charset_Size5(pass) + 
         //" characters\n";
...
function CheckIfLoaded4()
{
   ...
   else if (! document.Common_Parsed)
   {
      ...
      {
         window.setTimeout('Parse_Common4()', 50);
   ...
   {
      Set_Text4(s + "Loading ...");
      window.setTimeout('CheckIfLoaded4()', 200);
      return;
   }
   
   // Loaded. Do initialization thingies.
   Set_Text4("Finished Loading.");
   window.setTimeout('ShowStats4();', 1000);
}

window.setTimeout('CheckIfLoaded4()', 100);
