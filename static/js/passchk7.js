...
function Parse_Common7()
{
   ...
   {
      window.setTimeout('Parse_Common7()', 20);
   }
   else
   {
...
// number for "qu".
function Parse_Frequency_Token7()
{
   var c;
...
function Get_Charset_Size7(pass)
{
...
function Set_Text7(s)
{
   ...
   e = document.getElementById('passchk_result7');
   if (! e)
   {
      return;
   }
...
function ShowStats7()
{
   ...
   if (pass == OldPass7)
   {
      window.setTimeout('ShowStats7();', 200);
      return;
   }
   OldPass7 = pass;
   ...
   if (pass.length > 1)
   {
      ...
      charSet = Math.log(Get_Charset_Size7(pass)) / Math.log(2);
      aidx = Get_Index7(plower.charAt(0));
      for (var b = 1; b < plower.length; b ++)
      {
	 var bidx = Get_Index7(plower.charAt(b));
...
function CheckIfLoaded7()
{
   ...
   else if (! document.Common_Parsed)
   {
      ...
      {
         window.setTimeout('Parse_Common7()', 50);
   ...
   {
      Set_Text7(s + "Loading ...");
      window.setTimeout('CheckIfLoaded7()', 200);
      return;
   }
   
   // Loaded. Do initialization thingies.
   Set_Text7("Finished Loading.");
   window.setTimeout('ShowStats7();', 1000);
}

window.setTimeout('CheckIfLoaded7()', 100);
