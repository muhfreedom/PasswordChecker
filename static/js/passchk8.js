...
function Parse_Common8()
{
   ...
   {
      window.setTimeout('Parse_Common8()', 20);
   }
   else
   {
...
// number for "qu".
function Parse_Frequency_Token8()
{
   var c;
...
function Get_Charset_Size8(pass)
{
...
function Set_Text8(s)
{
   ...
   e = document.getElementById('passchk_result8');
   if (! e)
   {
      return;
   }
...
function ShowStats8()
{
   var pass = document.passchk_form.passchk_pass3.value;
   var plower = pass.toLowerCase();
   var r = "";
   
   if (pass == OldPass8)
   {
      window.setTimeout('ShowStats8();', 200);
      return;
   }
   OldPass8 = pass;
   ...
   if (pass.length > 1)
   {
      ...
      charSet = Math.log(Get_Charset_Size8(pass)) / Math.log(2);
      aidx = Get_Index8(plower.charAt(0));
      for (var b = 1; b < plower.length; b ++)
      {
	 var bidx = Get_Index8(plower.charAt(b));
...
function CheckIfLoaded8()
{
   ...
   else if (! document.Common_Parsed)
   {
      ...
      {
         window.setTimeout('Parse_Common8()', 50);
   ...
   {
      Set_Text8(s + "Loading ...");
      window.setTimeout('CheckIfLoaded8()', 200);
      return;
   }
   
   // Loaded. Do initialization thingies.
   Set_Text8("Finished Loading.");
   window.setTimeout('ShowStats8();', 1000);
}

window.setTimeout('CheckIfLoaded8()', 100);
